...
ASFR_graph <- ASFR_Kenya_Sweden_comparison %>%
  mutate(age = kenya_data$age,
         period = kenya_data$period) %>%
  pivot_longer(values_to = "ASFR", names_to = "Region", cols = 1:2) %>%
  mutate(lower_age = as.numeric(str_extract(age, "^\\d+"))) %>%  
  filter(lower_age >= 4 & lower_age < 55) # only select the correct age range

# correct order of ages shown
ASFR_graph$age <- factor(ASFR_graph$age, 
                         levels = gtools::mixedsort(unique(ASFR_graph$age)))

ggplot(data = ASFR_graph, 
